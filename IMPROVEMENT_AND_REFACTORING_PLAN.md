...
- [x] слушать `127.0.0.1` по умолчанию через явный `HOST`;
...
**Статус:** 🟡 выполняется (HTTP-ядро готово, миграция legacy-маршрутов продолжается).
...
- [x] внедрить простой декларативный router (`router.js`, первый маршрут `/api/health`);
...
- [ ] все legacy-маршруты переведены на единый JSON/error contract;
- [x] malformed JSON на мигрированных маршрутах всегда возвращает 400;
...
- `server.js` заменён компактными `src/app.js` и `src/main.js`;
- route modules не обращаются напрямую к process env или файловой системе;
- сервисы получают зависимости через параметры;
- timer и store закрываются через lifecycle API;
- smoke-запуск и все интеграционные тесты проходят.
...
- вынести криптографию и persistence в отдельные компоненты;
- добавить проверочную запись master key;
- различать not-found, corrupted и wrong-key;
- ввести schema/allowlist ключей вместо произвольного KV API;
- реализовать batch transaction на уровне сервиса;
- добавить `flush()` и `close()`;
- документировать backup/restore и ротацию ключа;
- рассмотреть переход на нативный SQLite только при появлении требований к объёму/конкурентности.

**Критерии готовности:**

- повреждение записи и неверный ключ обнаруживаются явно;
- частично сохранённый набор настроек невозможен;
- произвольное имя ключа нельзя записать через публичный API;
- есть unit-тесты повторного открытия и конкурентных записей.
...
- перейти на `<script type="module">`;
- вынести API client и settings client;
- вынести formatters и model matching как чистые модули;
- выделить settings dialog, banner и topbar;
- перенести страницы по одной: Models → Combo → Statistics → Cheatsheet;
- удалить глобальные изменяемые ссылки на DOM там, где возможна локальная инициализация;
- перестать подавлять ошибки сохранения/загрузки;
- убрать секреты из browser cache после отправки.

**Критерии готовности:**

- у каждой страницы отдельный entry/controller;
- изменение Models не требует загрузки или понимания Antigravity/Combo логики;
- API ошибки показываются единообразно;
- чистая логика имеет unit-тесты;
- функциональность страниц не изменилась визуально.
...
1. AgentRouter models/usage windows как расширение адаптера.
