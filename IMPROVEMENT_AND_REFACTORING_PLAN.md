...
**Статус:** ✅ выполнен 2026-09-03 (закрыты порог покрытия, аудит зависимостей и проверка документации — детали в разделе 11.1, Этап 10).
...
- [x] установить разумные coverage thresholds только для новых модулей (`test:ci`: 95/95/95 для `src/**`, `public/js/**`, `security.js`, `http.js`, `router.js`; факт — 100/100/100);
- [x] добавить dependency audit (CI job `deps`: `npm audit --omit=dev --audit-level=high`) и проверку утечки секретов (`npm run check:secrets`);
- [x] проверять документацию/пример env на актуальность (`npm run lint:docs`).

**Критерии готовности:**

- [x] pull request не проходит при нарушении API/security regression;
- [x] smoke-тесты открывают все страницы и выполняют ключевые сценарии;
- [x] новые модули имеют согласованный порог покрытия;
- [x] CI не использует реальные внешние API и секреты.
...
## 11. Ближайшее продолжение плана (сентябрь–октябрь 2026)

После стабилизации базовых стадий и визуальной реорганизации логичнее закрыть оставшиеся инфраструктурные и продуктовые блоки без расширения API/функционала до полного закрытия рисков.

### 11.1. Этап 10. Довести этап 7 (CI/качество) до целевого состояния

**Статус:** ✅ выполнен 2026-09-03.

**Пороговая цель:** минимизировать регрессии между шагами фронтенд- и серверного рефакторинга.

**Работы:**

1. [x] Добавить покрытия для критичных зон в CI.
   - `npm run test:ci` (включает `--experimental-test-coverage` + `--test-coverage-include='src/**,public/js/**,security.js,http.js,router.js'` + `--test-coverage-lines/functions/branches=95`) — падает CI, если новые модули ниже 95% (факт: 100/100/100 на текущей кодовой базе);
   - пороги 95/95/95 выбраны как запас к фактическому показателю; устаревшие файлы `server.js`, `store.js`, `tests/` оставлены на отдельном `test:coverage` отчёте без блокировки.

2. [x] Проверка зависимостей и секретов.
   - отдельный CI-job `deps`: `npm audit --omit=dev --audit-level=high`;
   - `npm run check:secrets` (Node-скрипт `scripts/check-secrets.js`) — сканирует исходники на типичные шаблоны (64-hex, AIza…, AKIA…, sk-…, ghp_…), исключая `docs/`, `test/`, `tests/`, `.env*`; включён в job `lint`.

3. [x] Защищённая документация.
   - `npm run lint:docs` (`scripts/check-docs.js`) — сверяет наличие в `README.md`, `docs/BASELINE.md`, `.env.example` ключевых переменных окружения и базовых endpoints; встроен в CI job `lint`;
   - в плане зафиксированы статусы этапов; отдельный `CHANGELOG` пока не вводится — изменения делаются через git-коммиты и release notes после стабилизации.

4. [x] Единый набор quality gates в CI.
   - `lint` (eslint, stylelint, test:ci, lint:docs, check:secrets) → `check` (полный `npm test` на Node 20/22) → `smoke` (Playwright) + параллельно `security` (security-юниты) и `deps` (`npm audit`); добавлена зависимость `needs: lint` для блокирующих job;
   - для каждого job настроен `if: success()` и явный `actions/setup-node@v4` с кешем npm.

**Критерии готовности:**

- [x] PR/пуш не проходит при падении CI по критичному качеству;
- [x] новые модули защищены порогами покрытия (95% по всем трём показателям);
- [x] нет новых утечек секретов в репозитории и логах;
- [x] документация по env/API и baseline подтверждена в автоматизированной проверке.

### 11.2. Этап 11. Доработки продукта (после закрытия этапа 10)

**Статус:** 🟡 идёт по приоритету, не блокирует безопасность.

**Приоритетный порядок:**

1. [ ] Множественные аккаунты и учётные данные через сущности `account`/`credential`.
2. [ ] Управление локальными процессами только в local-only режиме:
   - strict allowlist;
   - аудит операций;
   - ограничение времени выполнения и размер команды;
   - подтверждение действий пользователем.
3. [ ] Расширение notification service (каналы, дедупликация между вкладками, настройки порога, интеграция в health).

**Критерии готовности:**

- [ ] этапы продукта не влияют на DOD базовой безопасности и наблюдаемости;
- [ ] для каждой новой фичи есть отдельный ADR/ RFC и API/UX регрессия.

**Следующий кандидат на RFC-0003:** модель данных для нескольких аккаунтов (`account` + `credential`) в encrypted store; ожидаемый объём — 3–5 дней на серверную часть и 1–2 дня на UI.

**RFC-0003: Multi-Account Credentials — статус серверной части:** 🟡 выполнено 2026-09-03.

- Добавлена таблица `credentials` в encrypted store (`src/store/accounts.js`).
- CRUD: `listAccounts`, `createAccount`, `updateAccount`, `deleteAccount`, `resolveCredential`, `getActiveCredential`.
- Эндпоинты: `GET/POST /api/accounts`, `PUT/DELETE /api/accounts/:name`, `GET/PUT /api/accounts/active`.
- Провайдерские маршруты (`/api/providers/:id/usage`, `/models`) читают ключи активного аккаунта с фолбэком на legacy-поля `kv`.
- Активный аккаунт хранится в `kv.activeAccount` (добавлен в `STORE_KEYS`).
- Миграция: при первом открытии legacy-поля `xkiroKey`/`agentrouterKey`+`agentrouterUserId`/`omniUrl`+`omniKey`/`agRefreshToken`+`agProject`+`agEmail` переносятся в аккаунт `default`.
- Тесты: 15 unit + 8 integration (всего 23, `npm test` зелёный).

**Осталось по RFC-0003:** UI (1–2 дня) — селектор аккаунта в диалоге настроек, индикатор активного аккаунта в topbar, сохранение credentials для нового аккаунта.

### 11.3. Обновлённые критерии готовности всего плана

Добавляется финальный контроль перед тем, как считать проект «production-ready»:

- [x] CI содержит отдельный security gate (`audit` + CORS/secret checks + smoke);
- [x] README и `.env.example` синхронизированы с `docs/BASELINE.md` и проверяются автоматически;
- [x] все критические сценарии покрыты интеграционными, security и browser smoke тестами;
- [x] есть понятная процедура проверки зависимостей/секретов для каждого релиза;
- [x] все этапы 0–9 отмечены завершёнными, а новые задачи идут только через roadmap/issue.

---

*Примечание: если команда идёт быстрым темпом, этапы 10–11 можно выполнять параллельно с последним пунктом этапа 9 (process control), но только после утверждённой security-модели remote-режима.*
