...
const ENV_KEY = 'env-key-123';

test('getUsage: GET /v1/usage, JSON как есть, ключ из .env', async () => {
  const mock = await startMockUpstream();
  try {
    const provider = createXKiroProvider({ url: mock.url, apiKey: ENV_KEY });
    ...
    assert.equal(mock.seen[0].key, ENV_KEY);
  } finally {
    await mock.close();
  }
});

test('клиентский ключ приоритетнее ключа из .env', async () => {
  const mock = await startMockUpstream();
  try {
    const provider = createXKiroProvider({ url: mock.url, apiKey: ENV_KEY });
